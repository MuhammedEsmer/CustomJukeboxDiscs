...
public final class ClientUploadManager implements ModPayloads.ClientHandler {
    ...
    public void beginFromUrl(String url, String title, long inputFingerprint, Consumer<Component> status) {
        ...
        ClientPacketDistributor.sendToServer(new UrlUploadRequest(url, title, inputFingerprint));
    }
    ...
    public void begin(
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                pending = upload;
                ClientPacketDistributor.sendToServer(new UploadBeginRequest(
    ...
    private void upload(Pending current, UploadBeginResponse response) {
        try (InputStream input = Files.newInputStream(current.file)) {
            ...
            while ((count = input.read(buffer)) >= 0) {
                if (count == 0) continue;
                ClientPacketDistributor.sendToServer(new UploadChunk(
            ...
            ClientPacketDistributor.sendToServer(new UploadFinish(response.sessionId(), current.hash));
